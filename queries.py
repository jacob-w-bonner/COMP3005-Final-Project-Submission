...
def Q_1(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, AVG(shot.statsbomb_xg) as xg_average
                FROM player, shot JOIN event on shot.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                ...
                WHERE player.player_id = shot.player_id AND
                      seasons.season_name = '2020/2021' AND
                      competitions.competition_name = 'La Liga'
                GROUP BY player.player_name
                ORDER BY xg_average DESC;"""
...
def Q_2(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, COUNT(shot.player_id) as shots_made
                FROM player, shot JOIN event on shot.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
...
def Q_3(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, COUNT(shot.player_id) as first_time_shots
                FROM player, shot JOIN event on shot.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE player.player_id = shot.player_id AND
                      shot.first_time = true AND
                      competitions.competition_name = 'La Liga' AND
                      (seasons.season_name = '2020/2021' OR
                       seasons.season_name = '2019/2020' OR
                       seasons.season_name = '2018/2019')
                GROUP BY player.player_name
...
def Q_4(cursor, conn, execution_time):
    ...
    query = """ SELECT team.team_name, COUNT(event.event_type_id) as number_of_passes
                FROM team, event JOIN event_type ON event.event_type_id = event_type.event_type_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE event.event_type_id = event_type.event_type_id AND
                      event_type.name = 'Pass' AND
                      team.team_id = event.team_id AND
                      seasons.season_name = '2020/2021' AND
                      competitions.competition_name = 'La Liga'
...
def Q_5(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, COUNT(pass.player_recipient_id) as times_passed_to
                FROM player, pass JOIN event on pass.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE player.player_id = pass.player_recipient_id AND
                      competitions.competition_name = 'Premier League' AND
                      seasons.season_name = '2003/2004'
                GROUP BY player.player_name
                ORDER BY times_passed_to DESC"""
...
def Q_6(cursor, conn, execution_time):
    ...
    query = """ SELECT team.team_name, COUNT(event.event_type_id) as number_of_shots
                FROM team, event JOIN event_type ON event.event_type_id = event_type.event_type_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE event.event_type_id = event_type.event_type_id AND
                      event_type.name = 'Shot' AND
                      team.team_id = event.team_id AND
...
def Q_7(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, COUNT(pass.player_id) as through_ball_number
                FROM player, pass JOIN event on pass.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE player.player_id = pass.player_id AND
                      pass.through_ball = true AND
                      competitions.competition_name = 'La Liga' AND
                      seasons.season_name = '2020/2021'
                GROUP BY player.player_name
                ORDER BY through_ball_number DESC;"""
...
def Q_8(cursor, conn, execution_time):
    ...
    query = """ SELECT team.team_name, COUNT(pass.player_id) as through_ball_number
                FROM team, pass JOIN event on pass.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE team.team_id = event.team_id AND
                      pass.through_ball = true AND
                      competitions.competition_name = 'La Liga' AND
                      seasons.season_name = '2020/2021'
                GROUP BY team.team_name
...
def Q_9(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, COUNT(dribble.outcome) as successful_dribbles
                FROM player, dribble JOIN event on dribble.event_id = event.event_id
                JOIN matches ON event.match_id = matches.match_id
                JOIN seasons ON matches.season_id = seasons.season_id
                JOIN competitions ON matches.competition_id = competitions.competition_id
                WHERE player.player_id = dribble.player_id AND
                      dribble.outcome = 'Complete' AND
                      competitions.competition_name = 'La Liga' AND
                      (seasons.season_name = '2020/2021' OR
                       seasons.season_name = '2019/2020' OR
                       seasons.season_name = '2018/2019')
...
def Q_10(cursor, conn, execution_time):
    ...
    query = """ SELECT player.player_name, COUNT(dribble.outcome) as successful_dribbles
                ...
                WHERE player.player_id = dribble.player_id AND
                      ...
                      seasons.season_name = '2020/2021'
                GROUP BY player.player_name
                ORDER BY successful_dribbles ASC;"""
